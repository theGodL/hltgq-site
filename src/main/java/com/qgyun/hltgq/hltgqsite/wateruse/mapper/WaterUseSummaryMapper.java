 ...
 * lgwutj 执行水价(元/m³) / hsfvdh 应收水费。
 * <p>表单无「计算用水量」列（业主 2026-09-11 确认）：用水量按 应收水费 ÷ 执行水价 计算
 * （computedUsage = hsfvdh ÷ lgwutj，水价为空/0 时为 null，该条不计入用水量）。
 */
@Mapper
public interface WaterUseSummaryMapper {
     ...
     * @return 水费计算记录（computedUsage 为计算得出的用水量；单位换算在服务层处理）
     */
    @Select("SELECT ztmhwx AS \"feeNo\", fbnomc AS \"unitName\", pyyftf AS \"periodTime\", " +
            "hsfvdh::numeric / NULLIF(lgwutj::numeric, 0) AS \"computedUsage\", " +
            "lgwutj AS \"priceRaw\", hsfvdh AS \"feeRaw\" " +
